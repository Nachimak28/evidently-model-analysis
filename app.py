...
class LoadData(L.LightningWork):
    ...
    def run(self):
        self.train_df = Payload(pd.read_csv('resources/ba_cancer_train_df_with_preds.csv'))
        self.test_df = Payload(pd.read_csv('resources/ba_cancer_test_df_with_preds.csv'))

class LitApp(L.LightningFlow):
    def __init__(self, train_dataframe_path=None, test_dataframe_path=None, target_column_name=None, prediction_column_name=None, task_type=None) -> None:
        super().__init__()
        self.train_dataframe_path = train_dataframe_path
        self.test_dataframe_path = test_dataframe_path
        self.target_column_name = target_column_name
        self.predicton_column_name = prediction_column_name
        self.task_type = task_type
				# Flask won't return 
        self.evidently_model_analysis = EvidentlyModelAnalysis(parallel=True, cloud_compute=L.CloudCompute("default"))
    ...
    def run(self):
        ...
        if self.load_data_component.train_df and self.load_data_component.test_df:
            self.evidently_model_analysis.run(train_df=self.load_data_component.train_df, test_df=self.load_data_component.test_df)
